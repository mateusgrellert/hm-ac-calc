...
def parseOutput(resultsPath):
	...
	try:
		...
		return False
def parseACOutput(resultsPath):
	hmResults = open(resultsPath, 'r').readlines()
	results = [None, None, None, None]
	for line in hmResults:
		if '64x64' in line:
			results[0] = line.split('\t')[1:-1]
		elif '32x32' in line:
			results[1] = line.split('\t')[1:-1]
		elif '16x16' in line:
			results[2] = line.split('\t')[1:-1]
		elif '8x8' in line:
			results[3] = line.split('\t')[1:-1]
	if None not in results:
		return results
	else:
		print  'Could not parse results AC for ', resultsPath, '. Skipping.'
...
def makeACValuesFile(gopStructure):
	acFile = open('ACResults' + gopStructure + '.csv', 'w')
	print >> acFile, '\t\t'+'\t\t\t\t\t\t'.join(['Reference'] + Configuration.testNameList)
	print >> acFile, 'Sequence\tQP\t' + '\t'.join(['SAD\tSATD\tSSE\tTransf\tHalf\tQuarter']*(Configuration.N_TESTS+1))
	return acFile
